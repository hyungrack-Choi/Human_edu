...
public class ListController implements Initializable {
	...
	public void ListController(int curpage, int totalpage) {
	...
	public void handleBtnLeft(ActionEvent event) {
		if (currentPage > 1) {
			...
			Animation.slide(listStackPane.getChildren()
					.get(1), 0, 350, new EventHandler<ActionEvent>() {
				@Override
				public void handle(ActionEvent event) {
	...
	public void handleBtnRight(ActionEvent event) {
		if (currentPage < totalPage) {
			...
			ListView<BorderPane> listView = getList(currentPage);
			listStackPane.getChildren().add(listView);
			Animation.slide(listView, 350, 0, new EventHandler<
					ActionEvent>() {
				@Override
				public void handle(ActionEvent event) {
	...
	public void handleBtnHome(ActionEvent event) {
		Animation.fade(AppMain029.stackPane.getChildren()
				.get(1), 1, 0, new EventHandler<ActionEvent>() {
			@Override
			public void handle(ActionEvent event) {
	...
	private ListView<BorderPane> getList(int page) {
		...
		for (int i = start; i <= end; i++) {
			...
			Image image = new Image(
					getClass().getResource("../images/phone" + 
			((i < 10) ? ("0" + i) : i) + ".png").toString());
